...
class UFTopicRelayEndpoint : public rclcpp::Node {
    ...
    std::vector<std::string> split_tunnel_namespaces;   
...
public:
    UFTopicRelayEndpoint(const rclcpp::NodeOptions & options): rclcpp::Node("endpoint", options) {
        ...
        topic_prefix = declare_parameter<std::string>("topic_prefix", decltype(topic_prefix)());
        ...
        split_tunnel_namespaces = declare_parameter<std::vector<std::string>>("namespaces", std::vector<std::string>());
        ...
        for (const auto& n : this->get_topic_names_and_types()) {
            ...
            if(std::find(std::begin(topic_names),std::end(topic_names), topic_name) != std::end(topic_names)){
                ...
                if(loc == std::string::npos) {
                    ...
                    input_name = remove_prefix(topic_name, topic_prefix) + suffix;
                }else{
                    input_name = std::string(remove_prefix(std::string(topic_name).replace(loc, from_uf_name.size(), uf_name), topic_prefix));
                }
            ...
            if (auto drain_info = try_discover_drain(topic_name)) {
                if(!found){
                    ...
                    auto num_namespaces = split_tunnel_namespaces.size();
                    auto namespaces = std::vector<std::string>(num_tunnels, "");
                    if (num_namespaces != num_tunnels) {
                        for (uint8_t i = 0; i<num_tunnels; i++){
                            namespaces[i] = split_tunnel_namespaces[0];
                        }
                    }
                    else {
                        namespaces = split_tunnel_namespaces;
                    }
                    RCLCPP_INFO(this->get_logger(), "Added endpoint: %s -> %s", found->info.input_topic.c_str(),found->info.output_topic.c_str());
                    for(uint8_t i = 0; i<num_tunnels; i++){
                        auto full_name = namespaces[i]+remove_prefix(input_name, topic_prefix)+split_tunnel_suffixes[i];
                        RCLCPP_INFO(this->get_logger(), "%d: with split tunnel suffix %s", i, split_tunnel_suffixes[i].c_str());
                        RCLCPP_INFO(this->get_logger(), "%d: with split tunnel namespace %s", i, namespaces[i].c_str());
                    }
                }
                ...
                if (!found->subs[0]) {
                    ...
                    if (num_tunnels == 0) {
                        ...
                        for(uint8_t i = 0; i<num_tunnels; i++){
                            auto full_name = namespaces[i]+found->info.input_topic+split_tunnel_suffixes[i];
