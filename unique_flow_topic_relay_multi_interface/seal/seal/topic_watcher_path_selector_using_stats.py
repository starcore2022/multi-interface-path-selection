...
from unique_flow_topic_relay.msg import TunnelSelect
...
class MinimalPublisher(Node):
    ...
    def __init__(self):
        ...
        self.publisher = self.create_publisher(TunnelSelect, 'select', 10)
        timer_period = 20  # seconds
        self.timer = self.create_timer(timer_period, self.timer_callback)

    # For dumb path switching periodically
    def timer_callback(self):
        ...
        msg.select = self.path
        self.publisher.publish(msg)
        # self.get_logger().info('Publishing: "{"select: %d"}"' % msg.data)
        self.get_logger().info('Publishing: '+"{"+'select: %d'% msg.select +"}" )


#For automatic switching based on the stats
class MinimalPublisher_using_stats(Node):
    path = 0

    def __init__(self):
        super().__init__('minimal_publisher_using_stats')
        self.publisher_ = self.create_publisher(TunnelSelect, 'select', 10)
...
# This function is a placeholder for switching channels.
def switch_channel(stats, publisher):

    if 1/stats.window_rate > 0.05: #50ms
        # Switch to a lower delay path
        if publisher.path == 0:
            publisher.path = 1
        else:
            publisher.path = 0
        ...
        msg = TunnelSelect()
        msg.data = publisher.path
        publisher.publish(msg)
        # self.get_logger().info('Switching channel to: %d' % msg.data)
        publisher.get_logger().info('Switching channel to: ' + "{"+('select: %d'% msg.data) +"}")
        return True
    
    # if 1/stats.window_average_delta > 0.05: #50ms
    #     # Switch to a lower delay path
    #     if publisher.path == 0:
    #         publisher.path = 1
    #     else:
            ...
            if res is not None:
                ...
                for publisher in pub_info_by_pub:
                    ...
                    if seal_sm.state == 'connection_in_group':
                        ...
                        self.networkexposure.NetworkResourceAdaptation("GROUP_ID", qci, uplinkMaxBitRate, 10, destinationIPv4, destinationPort, sourcePort, "TCP", "bidirectional")
